...
namespace Translator.Lexer
{
    public class LexerValidator
    {
        ...
        public void ValidateLabels(IList<Token> tokens)
        {
            ...
            {
                _lexer.Log(LogEventLevel.Warning, "Unused label: {0} at line {1}", false, labelToken.Name, labelToken.Line);
            }

            var undefined = labelUsages.Where(us => labels.All(x => us.Substring != x.Name));
            foreach (var labelToken in undefined.OfType<Identifier>())
            {
                _lexer.Log(LogEventLevel.Error, "Undefined label: {0} at line {1}", false, labelToken.Name, labelToken.Line);
            }
        }
