...
namespace Parser
{
    public static class EnumeratorCloner
    {
        public static IEnumerator<T> Clone<T>(this IEnumerator<T> source) where T : class
        {
            ...
            var sourceTypeConstructor = sourceType.GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance, null,
                new[] {typeof(List<T>)}, null);
            ...
            var newInstance =
                sourceTypeConstructor.Invoke(new[] {nonPublicFields.First(x => x.Name == "list").GetValue(source)}) as
                    IEnumerator<T>;
